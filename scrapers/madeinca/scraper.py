...
class MadeInCanadaScraper(BaseScraper):
    ...
    def scrape(self, **kwargs) -> list[dict[str, Any]]:
        ...
        page = self._fetch(self.url)

        content_divs = page.css(".entry-content")
        content = content_divs[0] if content_divs else (page.find("article") or page.find("body"))
        if not content:
            content = page
        ...
        for el in content.find_all(["h2", "h3", "table"]):
            if el.tag == "h2":
                current_section = el.get_all_text().strip()
            elif el.tag == "h3":
                current_category = el.get_all_text().strip()
            elif el.tag == "table":
                rows = el.find_all("tr")
                if not rows:
                    continue
                headers = [h.get_all_text().strip().lower() for h in rows[0].find_all(["th", "td"])]
                if not any("brand" in h for h in headers):
                    continue
                for row in rows[1:]:
                    cols = row.find_all(["td", "th"])
                    if len(cols) >= 3:
                        brand = cols[0].get_all_text().strip()
                        products = cols[1].get_all_text().strip()
                        location = cols[2].get_all_text().strip()
